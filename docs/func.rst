...
**openpyweb** Provides bundles of in-bulit functions that support ``mvc`` app development,
...
.. code-block:: python
    ...
    from openpyweb.Functions.__ import __
...
.. code-block:: python
    ...
    from openpyweb.Functions.count import count
...
.. note::
    ...
    Agent module is a dependency, and it might no longer be maintain under the openpyweb repository,
    if you find this module useful and you want to keep using its features in your project.
    We recommend installation of ``pip install openpyweb_agent``.
...
.. code-block:: python
    ...
    from openpyweb.Functions.agent import os, browser
...
.. code-block:: python
    ...
    from openpyweb.Functions.ip import ip
...
Path
----
openpyweb ``path`` module handles  method ``path``, ``exist`` , ``public`` argument which return root **path** as string.

**Import Module**

.. code-block:: python

    from openpyweb.Functions.path import path
...
openpyweb ``url`` module handles uniform resource locator notation using ``url`` method with ``path`` argument which returns the application link  
...
Iteration
---------
openpyweb iteration module handles ``iteration``, enumerate dictionary and ``Json``.

**Import Module**

.. code-block:: python

    from openpyweb.Functions.iteration import iteration
...
openpyweb ``curl`` is an in-built module support sending or initiating actions within or outside openpyweb framework.
...
.. code-block:: python
    ...
    from openpyweb.Functions.curl import curl
...
it happens every date, openpyweb provides the best way to handle time date and format with additional
future like readable time and date. now module contains methods that support ``ago``,  ``time``, ``date``, ``datetime``,
``create``, ``timestamp``,  ``past``, ``future``, ``subtract`` Now module is usable on both openpyweb template
...
openpyweb has a wonderful module that handles both including and extending of external file or paging ``include``
...
This modules are mostly used when working with openpyweb Template Engine or html pages
...
.. code-block:: python
    ...
    from openpyweb.Functions.extend import extend
    extending = extend()
...
openpyweb  ``validation`` module provides bundles of validity functions that help to validate and trim syntax, string and characters.
...
openpyweb provides pagination module that helps to navigate through pages and tables,
...
.. code-block:: python
    ...
    from openpyweb.Functions.pagination import  pagination
