...
openpyweb framework supports python version from 2.7 to newer,
openpyweb can be download from  ``https://pypi.org/openpyweb``  using terminal or command line promts
``pip install openpyweb``  or   ``https://github.com/openpyweb/openpyweb``
The installation will automatically be extracted into ``site-packages``  in Python  directory.

openpyweb provides command line tool that helps in creating project folder structure with the use of ``openpyweb-start``
...
download environment and openpyweb requirement.
